...
class Application extends Container
{
    ...
    {
        $authLockFile = base_path('storage/framework/cache/') . 'authLock.lock';
        if (is_file($authLockFile)) {
            $fileInfo = file_get_contents($authLockFile);
            $cache = json_decode($fileInfo, true);
        }
        $lifetime = 60 * 60;
        if (isset($cache['created']) && (time() - $cache['created'] > $lifetime)) {
            // 超过有效期，删除文件
            unlink($authLockFile);
            unset($cache);
        }
        if (isset($cache['no_auth']) && $cache['no_auth']) {
            $data = json_decode($cache['no_auth'],true);
            if (!$data['is_auth']){
                echo $data['msg'];exit();
                return false;
            }
        }
        if (isset($cache['lock']) && $cache['lock']) {
            return true;
        }
        ...
        $auth_key = env('AUTH_KEY', config('app.auth_key'));
        $auth_url = env('AUTH_URL', config('app.auth_url'));
        $client = new Client(['verify' =>false]);
        $response = $client->get($auth_url."/auth/domain/{$auth_key}");
        $body = $response->getBody();
        $content = (string) $body;
        $data = json_decode($content, true);
        if (!$data['data']['is_auth']){
            $cache['no_auth'] = json_encode($data['data']);
            @file_put_contents($authLockFile, json_encode($cache));

            echo $data['data']['msg'];exit();
            return false;
        }
        @file_put_contents($authLockFile, json_encode($cache));
...
//            session_destroy();
//        }
//        if (isset($_SESSION['no_auth']) && $_SESSION['no_auth']) {
//            $data = json_decode($_SESSION['no_auth'],true);
//            if (!$data['is_auth']){
//                echo $data['msg'];exit();
//                return false;
//            }
...
//        $data = json_decode($content, true);
//        if (!$data['data']['is_auth']){
//            $_SESSION['no_auth'] = json_encode($data['data']);
//            echo $data['data']['msg'];exit();
//            return false;
//        }

//        if (isset($_COOKIE['no_auth']) && $_COOKIE['no_auth']) {
//            $data = json_decode($_COOKIE['no_auth'],true);
//            if (!$data['is_auth']){
//                echo $data['msg'];exit();
//                return false;
//            }
//        }
//        if (isset($_COOKIE['lock']) && $_COOKIE['lock']) {
//            return true;
//        }
//        setcookie('lock', '1', time()+60, '/');
//        $auth_key = env('AUTH_KEY', config('app.auth_key'));
//        $auth_url = env('AUTH_URL', config('app.auth_url'));
//        $client = new Client(['verify' =>false]);
//        $response = $client->get($auth_url."/auth/domain/{$auth_key}");
...
//        $data = json_decode($content, true);
//        if (!$data['data']['is_auth']){
//            setcookie('no_auth', json_encode($data['data']), time()+60, '/');
//            echo $data['data']['msg'];exit();
//            return false;
//        }
    }
